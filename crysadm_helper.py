...
def save_history(username):
    ...
    for user_id in r_session.smembers('accounts:%s' % username):
        ...
        for device in data.get('device_info'):
            ...
            this_speed += int(int(device.get('dcdn_clients')[0].get('upload_speed')) / 1024)                #20181206更新
            #today_data['last_speed'] += int(int(device.get('dcdn_upload_speed')) / 1024)
            today_data['last_speed'] += int(int(device.get('dcdn_clients')[0].get('upload_speed')) / 1024)  #20181206更新
            #today_data['deploy_speed'] += int(device.get('dcdn_download_speed') / 1024)
            today_data['deploy_speed'] += int(device.get('dcdn_clients')[0].get('download_speed') / 1024)   #20181206更新
    ...
    #r_session.setex(key, json.dumps(today_data), 3600 * 24 * 35)   ###此处有问题 待检查
    r_session.setex(key, 3600 * 24 * 35, json.dumps(today_data))    ###20181207更新
...
def save_income_history(username, pdc_detail):
    ...
    r_session.setex(key, 3600 * 72, json.dumps(income_history))   ##20181207更新
