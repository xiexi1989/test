...
def dashboard_speed_share():
    ...
    for user_id in sorted(r_session.smembers('accounts:%s' % username)):
        ...
        for device in data.get('device_info'):
            ...
            uploadspeed = int(int(device.get('dcdn_clients')[0].get('upload_speed')) / 1024)  # 最新速度获取
...
def dashboard_speed_detail():
    ...
    for user_id in sorted(r_session.smembers('accounts:%s' % username)):
        ...
        for device in data.get('device_info'):
            if device.get('status') != 'online': continue
            upload_speed = int(int(device.get('dcdn_clients')[0].get('upload_speed')) / 1024) #最新速度获取
            deploy_speed = int(device.get('dcdn_clients')[0].get('download_speed') / 1024)  #最新速度获取
