...
def ubus_cd(session_id, account_id, out_params, url_param=None):
    ...
    try:
        ...
        return __handle_exception(e=e)
'''
#新版本 20181206更新 获取星域存储相关信息
def ubus_cd(session_id, account_id, out_params, url_param=None):
    ubus_id = int(time.time() * 1000)
    url = "http://ocapi.peiluyou.com:8008/ubus_cd?account_id=%s&session_id=%s&action=ubus_%s" % (
    account_id, session_id, ubus_id)

    body = {'id': 1, 'jsonrpc': '2.0', 'method': 'call',
            'params': [session_id, 'server', 'get_devices', {}]}
